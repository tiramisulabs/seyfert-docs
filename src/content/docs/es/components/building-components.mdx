...
```ts {"Establece todos los componentes dentro de esta ActionRow": 4-5} {"Añadir componentes a la ActionRow": 6-7} showLineNumbers copy
...
```ts {"Menú select de cadenas": 10-19} {"Menú select de usuarios": 20-27} {"Menú select de roles": 28-35} {"Menú select de canales": 36-45} {"Menú select de mencionables": 46-58} showLineNumbers copy
...
const mentionableMenu = new MentionableSelectMenu()
  ...
  .setDefaultMentionables(
    { type: 'Usuario', id: '123456789' },
    { type: 'Rol', id: '987654321' }
  );
