...
Todos los constructores se envían dentro de una [`ActionRow`](/api/classes/actionrow). Cada mensaje puede contener un máximo de 5 acciones fila.
...
Hay diferentes tipos de Select Menu: [`StringSelectMenu`](/api/classes/stringselectmenu), [`UserSelectMenu`](/api/classes/userselectmenu), [`RoleSelectMenu`](/api/classes/roleselectmenu), [`ChannelSelectMenu`](/api/classes/channelselectmenu) y [`MentionableSelectMenu`](/api/classes/mentionableselectmenu).
...
Después de crear los componentes tendrá que enviar el ActionRow al campo de componentes. [Ver más abajo](/misc/sending-messages#sending-components-attached-to-the-message).