...
Para ello es necesario crear un fichero llamado `seyfert.config.mjs` en la carpeta raíz de tu proyecto y añadir el siguiente código:
...
// Esto iniciará la conexión con la gateway de Discord y cargará comandos, eventos, componentes y lenguajes (i18n)
client.start();
```
    </TabItem>
    <TabItem label="HTTP">
...
// Esto empezará a escuchar `https://yourweb.xyz/interactions` y cargará comandos, componentes y lenguajes (i18n)
client.start();
```
    </TabItem>
</Tabs>
