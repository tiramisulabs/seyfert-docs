...
Seyfert no solo maneja comandos de aplicación, sino también `comandos de prefijo`. No están habilitados por defecto, pero puedes habilitarlos configurando una opción dentro de [`ClientOptions`](https://github.com/tiramisulabs/seyfert/blob/455ed12b0ebcb3ddf55bc8b3274b0ce904becc62/src/client/client.ts#L230). We have to access to the `command` option where we can specify a suboption called `prefix`. This option expects a callback returning an array of possible prefixes for that command. As the callback returns a [`Message`](https://github.com/tiramisulabs/seyfert/blob/455ed12b0ebcb3ddf55bc8b3274b0ce904becc62/src/structures/Message.ts#L123). Tenemos acceso a la opción `command` donde podemos especificar una subopción llamada `prefix`. Esta opción espera un callback que devuelve un array de posibles prefijos para ese comando. Como el callback devuelve un [`Message`](https://github.com/tiramisulabs/seyfert/blob/455ed12b0ebcb3ddf55bc8b3274b0ce904becc62/src/structures/Message.ts#L123) property inside [`CommandContext`](https://github.com/tiramisulabs/seyfert/blob/455ed12b0ebcb3ddf55bc8b3274b0ce904becc62/src/commands/applications/chatcontext.ts#L24) es bastante simple manejar diferentes prefijos por servidor.
