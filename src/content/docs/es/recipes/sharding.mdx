...
## Cache
A diferencia de lo que pueden ser otras librerias, al tener el sharding totalmente unificado seyfert mantiene el poder en algunas areas, entre ellas `el cache`.
Puedes hacer que el cache se centralize en el proceso principal, digase el ejecutor de `WorkerManager`, no importa en que parte del codigo estes, siempre tendras acceso a todos los datos.
Para esto, es necesario usar el WorkerAdapter:
```ts twoslash
...
```ts
// Ojalá typescript permitiera representar parametros como python
client.tellWorker(
	workerId = 1,
