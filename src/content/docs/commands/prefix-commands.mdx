...
```ts showLineNumbers copy ins={4-9}
...
Now that we have set up our prefixes for the commands it's high time we want to make them run. 

And for your luck, is the same that if we handle application commands. The only difference is that we receive an optional [`Message`](/api/classes/message) property inside [`CommandContext`](/api/classes/commandcontext).

To make this property appear in our CommandContext the only thing we shall do is declaring seyfert module and set the `withPrefix` property inside [`InternalOptions`](/api/interfaces/internaloptions) into true. You can see this [detailed example](/guides/declare-module#internal-options). 
...
export default class CrosspostCommand extends Command {
    async run(ctx: CommandContext){
        ...
        return await ctx.write({ content: 'I have cross posted your announcement.'});
    }
};
```
...
Like `application commands` you can set options for `prefix commands`. The only think you will have to do it's to declare them as you do in `application commands`. They are parsed with a regex that indicates options will be splited when they are followed by `-`. You can replace the parser with your own setting the `argsParser` suboption inside `commands` options in client options. This is a callback whose params are `content`, the content of the message, and `command`, the command which was run.

Here is a simple example of how we make the previous command but instead of cross posting the current command cross posing another message, fetching it by its id.
...
export default class CrosspostCommand extends Command {
    async run(ctx: CommandContext<typeof options>){
        ...
        const message = await ctx.client.messages.fetch(id, ctx.channelId);
...
export default class CrosspostCommand extends Command {
    async run(ctx: CommandContext<typeof options>){
        ...
        const { id } = ctx.options;
        await ctx.client.messages.crosspost(id, ctx.channelId);
        return await ctx.editOrReply({ content: 'I have cross posted your announcement.'});
    }
};
```
</TabItem>
