...
:::note
This section will assume that you read and understood [Language Management](/getting-started/languages/#creating-a-language-file).
:::
...
> There are some [decorators](https://www.typescriptlang.org/docs/handbook/decorators.html) that can be used to use the locales in your code.
...
@LocalesT('some.command.name', 'some.command.description')
...
const options = {
    supremacy: createStringOption({
        ...
        locales: {
            name: "some.command.option.name",
            description: "some.command.option.description",
        }
    })
}
```
