...
```ts twoslash {10} title="seyfert.config.mjs" showLineNumbers
// @ts-check
...
```ts twoslash title="src/commands/ping.ts" showLineNumbers
...
```ts twoslash {5-5} title="src/index.ts" showLineNumbers
...
```ts twoslash {4-5,10-14,20,23-24} title="src/commands/ping.ts" showLineNumbers
import type { Client, ParseClient } from 'seyfert';
