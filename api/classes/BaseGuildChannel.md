[seyfert](../globals.md) / BaseGuildChannel

# Class: BaseGuildChannel

## Extends

- `ObjectToLower`\<`Omit`\<`APIGuildChannel`\<`ChannelType`\>, `"permission_overwrites"`\>\>.[`BaseChannel`](BaseChannel.md)\<`ChannelType`\>
...
```ts
new BaseGuildChannel(client: UsingClient, data: APIGuildChannel<ChannelType>): BaseGuildChannel
```
