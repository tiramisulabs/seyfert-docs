[seyfert](../globals.md) / StageChannel

# Class: StageChannel

## Extends

- `ObjectToLower`\<`Omit`\<`APIGuildStageVoiceChannel`, `"type"`\>\>.[`TopicableGuildChannel`](TopicableGuildChannel.md).[`VoiceChannelMethods`](VoiceChannelMethods.md).[`BaseChannel`](BaseChannel.md)\<`ChannelType`\>
...
```ts
new StageChannel(client: UsingClient, data: APIChannelBase<ChannelType>): StageChannel
```
...
| `guildId` | `public` | `undefined` \| `string` | [`VoiceChannelMethods`](VoiceChannelMethods.md).`guildId` |
...
| `type` | `public` | `GuildStageVoice` | [`BaseChannel`](BaseChannel.md).`type` |
